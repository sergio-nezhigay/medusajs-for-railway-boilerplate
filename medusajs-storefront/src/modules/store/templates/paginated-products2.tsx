...
import ProductPreview2 from "@modules/products/components/product-preview2"
...
export default async function PaginatedProducts2({
  sortBy,
  page,
  collectionId,
  categoryId,
    ...
    <>
      <ul className="grid grid-cols-2 w-full small:grid-cols-3 medium:grid-cols-4 gap-x-6 gap-y-8">
        {products.map((p) => {
          return (
            <li key={p.id}>
              <ProductPreview2 productPreview={p} region={region} />
            </li>
          )
        })}
      </ul>
